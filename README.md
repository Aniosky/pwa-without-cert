...
Текущая версия хранится в трех местах:

- `sw.js` в константе `VERSION`
- `login.html` в `data-login-version` и pill `v...`
- `business-error.html` в `data-error-version` и pill `v...`

При изменении поведения PWA нужно синхронно поднять версию во всех трех файлах. На экране логина и fallback-экране версия видна в верхнем блоке, чтобы на iPhone было понятно, обновился ли service worker и HTML.
